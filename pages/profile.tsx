...
export default function Profile() {
    ...
    const fetchProfileDetails = async () => {
      ...
      try {
        const response = await fetch(
          `https://mango-transaction-log.herokuapp.ca/v3/user-data/profile-details?profile-name=${name
            ?.toString()
            .replace(/-/g, ' ')}`
        )
  ...
  const fetchFollowers = async () => {
    ...
    try {
      const followerRes = await fetch(
        `https://mango-transaction-log.herokuapp.ca/v3/user-data/followers?wallet-pk=${pubkey}`
      )
  ...
  const fetchFollowing = async () => {
    ...
    try {
      ...
      const followingRes = await fetch(
        `https://mango-transaction-log.herokuapp.ca/v3/user-data/following?wallet-pk=${profilePk}`
      )
  ...
  const fetchAccountStats = async (pk) => {
    const response = await fetch(
      `https://mango-transaction-log.herokuapp.ca/v3/stats/account-performance-detailed?mango-account=${pk}&start-date=${dayjs()
        .subtract(1, 'day')
        .format('YYYY-MM-DD')}`
    )
...
const EditProfileModal = ({
  ...
  const validateProfileName = async (name) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://mango-transaction-log.herokuapp.ca/v3/user-data/check-profile-name-unique?profile-name=${name.toLowerCase()}`
      )
  ...
  const saveProfile = async () => {
    ...
    if (!Object.keys(inputErrors).length) {
      ...
      try {
        ...
        const response = await fetch(
          'https://mango-transaction-log.herokuapp.ca/v3/user-data/profile-details',
          requestOptions
        )
