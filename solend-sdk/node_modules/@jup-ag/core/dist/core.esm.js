import { u64, TOKEN_PROGRAM_ID, NATIVE_MINT, ASSOCIATED_TOKEN_PROGRAM_ID, Token as Token$1 } from '@solana/spl-token';
import { PublicKey, MAX_SEED_LENGTH, SYSVAR_RENT_PUBKEY, SystemProgram, TransactionInstruction, Transaction } from '@solana/web3.js';
...
import { ZERO as ZERO$1, TokenSwapConstantProduct, Fraction, ZERO_FRACTION, Stable, TWO, ONE, TokenSwapStable, toDecimal } from '@jup-ag/math';
import Decimal from 'decimal.js';
import fetch from 'cross-fetch';
import { deserializeAccount, TransactionError, parseErrorForTransaction, UNKNOWN_ERROR } from '@mercurial-finance/optimist';
export { TransactionError } from '@mercurial-finance/optimist';
...
import { getTickArrayPks, parseTickArray, getSwapQuote, getDefaultSqrtPriceLimit, getTickArrayPublicKeysForSwap, parseWhirlpool } from '@jup-ag/whirlpool-sdk';
...
const INDEXED_ROUTE_MAP_URL = 'https://cache.jup.ag/indexed-route-map';
...
const MARINADE_PROGRAM_ID = /*#__PURE__*/new PublicKey('MarBmsSgKXdrN1egZf5sqe1TMai9K1rChYNDJgjq7aD');
const STEPN_PROGRAM_ID = /*#__PURE__*/new PublicKey('Dooar9JkhdZ7J3LHN3A7YCuoGRUggXhQaG4kijfLGU2j');
const ORCA_PROGRAM_ID = /*#__PURE__*/new PublicKey('9W959DqEETiGZocYWCQPaJ6sBmUzgfxXfqGeTEdp3aQP');
const STEP_TOKEN_SWAP_PROGRAM_ID = /*#__PURE__*/new PublicKey('SSwpMgqNDsyV7mAgN9ady4bDVu5ySjmmXejXvy2vLt1');
const PENGUIN_PROGRAM_ID = /*#__PURE__*/new PublicKey('PSwapMdSai8tjrEXcxFeQth87xC4rRsa4VA5mhGhXkP');
const SAROS_PROGRAM_ID = /*#__PURE__*/new PublicKey('SSwapUtytfBdBn1b9NUGG6foMVPtcWgpRU32HToDUZr');
...
function createProgramAddressSyncUnsafe(seeds, programId) {
  ...
  let publicKeyBytes = new BN(hash, 16).toArray(undefined, 32); // if (PublicKey.isOnCurve(new Uint8Array(publicKeyBytes))) {
...
const JUPITER_PROGRAM_ID_PRODUCTION = /*#__PURE__*/new PublicKey('JUP3c2Uh3WA4Ng34tw6kPd2G4C5BB21Xo36Je1s32Ph');
...
const PRODUCTION_TOKEN_LEDGERS = [/*#__PURE__*/new PublicKey('755CiAfB63jK8DTZSM38ZRBTjf1inGM4QfLJTfpPM9x3'), /*#__PURE__*/new PublicKey('5ZZ7w2C1c348nQm4zaYgrgb8gfyyqQNzH61zPwGvEQK9'), /*#__PURE__*/new PublicKey('H4K65yLyYqVsDxgNCVGqK7MqrpKFLZjmqf95ZvmfyVDx'), /*#__PURE__*/new PublicKey('HE4STzYv5dzw2G374ynt4EYvzuKLG41P2xnNffzpdWnG'), /*#__PURE__*/new PublicKey('3HmXTbZf6G2oEjN3bPreZmF7YGLbbEXFkgAbVFPaimwU'), /*#__PURE__*/new PublicKey('CUNMrNvGNh1aWR6cVzAQekdsW2dfacnQicyfvgvrN5ap'), /*#__PURE__*/new PublicKey('6Q6vMHsUFA7kuwdkG9vm7gByMfk151Z9eMSwE14fHcrG')];
...
const [ammAuthority] = /*#__PURE__*/findProgramAddressSync([/*#__PURE__*/new Uint8Array( /*#__PURE__*/Buffer.from( /*#__PURE__*/'amm authority'.replace('\u00A0', ' '), 'utf-8'))], RAYDIUM_AMM_V4_PROGRAM_ID);
...
function createMarinadeFinanceDepositInstruction({
  ...
  const tempWsolTokenAccount = findProgramAddressSync([Buffer.from('temp-wsol-token-account'), transferFrom.toBuffer()], JUPITER_PROGRAM_ID)[0];
  const tempSolPda = findProgramAddressSync([Buffer.from('temp-sol-pda'), userTransferAuthority.toBuffer()], JUPITER_PROGRAM_ID)[0];
...
function createWhirlpoolSwapExactOutputInstruction({
  ...
  return JUPITER_PROGRAM.instruction.whirlpoolSwapExactOutput(outAmount, maximumInAmount, additionalArgs.aToB, (_platformFee$feeBps19 = platformFee === null || platformFee === void 0 ? void 0 : platformFee.feeBps) !== null && _platformFee$feeBps19 !== void 0 ? _platformFee$feeBps19 : 0, {
    ...
    remainingAccounts: prepareRemainingAccounts(new BN(0), tokenLedger, platformFee === null || platformFee === void 0 ? void 0 : platformFee.feeAccount)
  });
}
...
class RaydiumAmm {
  ...
  static decodeSerumMarketKeysString(serumProgramId, serumMarket, serumMarketInfo) {
    ...
    const serumVaultSigner = createProgramAddressSyncUnsafe([serumMarket.toBuffer(), decodedMarket.vaultSignerNonce.toArrayLike(Buffer, 'le', 8)], serumProgramId);
    return {
...
function forecastBuy(market, orderbook, pcIn, takerFeePct) {
  let coinOut = ZERO$1;
  let bestPrice = ZERO$1;
  let worstPrice = ZERO$1; // total base price

  let totalCost = ZERO$1;
  let totalCoins = ZERO$1; // might be decimal, e.g: 0.001

  const quoteSizeLots = market.quoteSizeLotsToNumber(new BN(1)); // Serum buy order take fee in quote tokens

  let availablePc = quoteSizeLots ? JSBI.BigInt(new Decimal(pcIn.toString()).div(1 + takerFeePct).div(quoteSizeLots).floor()) : ZERO$1;
  ...
  return {
    ...
    minimum: {
      in: JSBI.BigInt(bestPriceSizeLots.mul(baseSizeLots.toString()).mul(1 + takerFeePct).ceil()),
      out: baseSizeLots
    },
...
function forecastSell(market, orderbook, coinIn, takerFeePct) {
  ...
  return {
    ...
    minimum: {
      in: baseSizeLots,
      out: JSBI.BigInt(bestPriceDecimal.mul(JSBI.toNumber(baseSizeLots)).mul(1 - takerFeePct).floor().toString())
    },
    inAmount: inAmount,
...
const HUNDRED = /*#__PURE__*/JSBI.BigInt(100); // Create an iteration to quote with a stepped split
...
class SplitTradeAmm {
  ...
  getQuote(quoteParams) {
    ...
    if (firstAmmFee.mint !== secondAmmFee.mint) {
      ...
      if (firstAmmFee.mint !== sourceMintString) {
        firstAmmFee = {
          amount: JSBI.divide(JSBI.divide(JSBI.multiply(firstAmmFee.amount, JSBI.multiply(amount, JSBI.BigInt(portion1))), HUNDRED), bestSolution.outAmount),
      ...
      if (secondAmmFee.mint !== sourceMintString) {
        secondAmmFee = {
          amount: JSBI.divide(JSBI.divide(JSBI.multiply(JSBI.multiply(secondAmmFee.amount, amount), JSBI.BigInt(portion2)), HUNDRED), bestSolution.outAmount),
          mint: sourceMintString
        };
      }
    }
  ...
  createSwapInstructions(swapParams) {
    ...
    const firstAmount = inAmount.mul(new BN$1(this.portion1)).div(new BN$1(HUNDRED.toString()));
    const secondAmount = inAmount.sub(firstAmount);
    return [...this.firstAmm.createSwapInstructions({ ...swapParams,
      amount: firstAmount,
      otherAmountThreshold: new BN$1(0),
      platformFee: undefined
    }), ...this.secondAmm.createSwapInstructions({ ...swapParams,
      amount: secondAmount,
      otherAmountThreshold: new BN$1(0),
      platformFee: undefined
    }), createRiskCheckAndFeeInstruction(swapParams.userDestinationTokenAccount, swapParams.userTransferAuthority, new BN$1(swapParams.otherAmountThreshold.toString()), swapParams.tokenLedger, swapParams.platformFee)];
  }
...
const PROGRAM_ID_TO_LABEL = /*#__PURE__*/new Map([[/*#__PURE__*/ORCA_PROGRAM_ID.toString(), 'Orca'], [/*#__PURE__*/STEP_TOKEN_SWAP_PROGRAM_ID.toString(), 'Step'], [/*#__PURE__*/PENGUIN_PROGRAM_ID.toString(), 'Penguin'], [/*#__PURE__*/SAROS_PROGRAM_ID.toString(), 'Saros'], [/*#__PURE__*/STEPN_PROGRAM_ID.toString(), 'Stepn']]);
...
function accountInfoToTokenSwapState(address, tokenSwapAccountInfo) {
  ...
  const authority = createProgramAddressSyncUnsafe([address.toBuffer(), Buffer.from([tokenSwapData.bumpSeed])], programId);
...
class CremaAmm {
  ...
  getQuote({
    ...
    const result = this.poolState.mintA.equals(sourceMint) ? this.preSwapA(new Decimal(amount.toString())) : this.preSwapB(new Decimal(amount.toString()));
...
class WhirlpoolAmm {
  ...
  createSwapInstructions(swapParams) {
    ...
    const [tickArray0, tickArray1, tickArray2] = getTickArrayPublicKeysForSwap(this.whirlpoolData.tickCurrentIndex, targetSqrtPrice, this.whirlpoolData.tickSpacing, this.address, this.tickArrays, WHIRLPOOL_PROGRAM_ID, aToB);
    const ix = swapParams.swapMode === SwapMode.ExactIn ? createWhirlpoolSwapInstruction({
      additionalArgs: {
        aToB,
...
const FEE_DENOMINATOR = /*#__PURE__*/JSBI.BigInt(1000000);
const provider = /*#__PURE__*/new anchor.AnchorProvider(null, null, {
  skipPreflight: false
});
...
class CykuraAmm {
  ...
  getQuote({
    ...
    const [currentOutAmount, newPool, swapAccountMetas] = this.pool.getOutputAmount(CurrencyAmount.fromRawAmount(inputToken, amount));
  ...
  createSwapInstructions(swapParams) {
    ...
    const [,, swapAccountMetas] = this.pool.getOutputAmount(CurrencyAmount.fromRawAmount(inputToken, swapParams.inAmount));
...
class MarinadeAmm {
  ...
  constructor(address, accountInfo) {
    ...
    this.id = address.toBase58();
    this.marinadeFinanceProgram = new Program(marinadeFinanceIdlSchema, MARINADE_PROGRAM_ID, {});
  ...
  update(accountInfoMap) {
    ...
    this.marinadeState = new MarinadeState(this.marinadeStateResponse, new BN(liqPoolSolLegPda.lamports), liqPoolMSOLLeg.amount);
  }
  ...
  getQuote({
    ...
    const result = sourceMint.equals(NATIVE_MINT) ? this.marinadeState.depositQuote(amountBN) : this.marinadeState.liquidUnstakeQuote(amountBN);
    return {
      notEnoughLiquidity: false,
      inAmount: amount,
  ...
  createSwapInstructions(swapParams) {
    return [swapParams.sourceMint.equals(NATIVE_MINT) ? createMarinadeFinanceDepositInstruction({ ...swapParams,
      additionalArgs: {
      ...
      inAmount: swapParams.amount,
      minimumOutAmount: swapParams.otherAmountThreshold
    }) : createMarinadeFinanceLiquidUnstakeInstruction({ ...swapParams,
      additionalArgs: {
        address: this.address,
        marinadeStateResponse: this.marinadeStateResponse,
        liqPoolSolLegPda: this.liqPoolSolLegPdaAddress
...
class MarinadeState {
  ...
  depositQuote(lamports) {
    ...
    let outAmountBN = new BN(0); // if we can sell from the LiqPool
  ...
  liquidUnstakeQuote(msolAmount) {
    ...
    const msolFee = msolAmount.mul(new BN(liquidUnstakeFeeBp)).div(new BN(10000));
...
function ammFactory(address, accountInfo, params) {
  ...
  if (programId.equals(MAINNET_SERUM_DEX_PROGRAM) || programId.equals(DEVNET_SERUM_DEX_PROGRAM)) {
    ...
    const stableSwap = StableSwap.loadWithData(address, accountInfo.data, createProgramAddressSyncUnsafe([address.toBuffer(), nonce], SWAP_PROGRAM_ID));
...
const mintCache = {}; // Since the mints are mostly repeated we want to avoid converting the same pk again
// This seems to bring getTokenRouteSegments from 100ms => 50ms

function getOrUpdatePublicKeyCache(pk) {
  //@ts-ignore
...
function computeInputRouteSegments({
  ...
  if (inputSegment && outputSegment) {
    ...
    for (let [mint, amms] of startSegment.entries()) {
      ...
      if (onlyDirectRoutes || swapMode === SwapMode.ExactOut || shouldSkipOutputMint(intermediateTokens, minSegmentSize, mint)) {
        continue;
      }
      ...
      if (intersectionAmms) {
        let filteredIntersectionAmms = swapMode === SwapMode.ExactIn ? intersectionAmms : intersectionAmms.filter(amm => amm.exactOutputSupported);
...
async function routeToInstructions({
  ...
  for (const [index, marketInfo] of routeInfo.marketInfos.entries()) {
    ...
    const legAmount = index === 0 ? new BN$1(amount.toString()) : null;
    const isLastLeg = index === legs - 1;
    const legOtherAmountThreshold = new BN$1((isLastLeg ? otherAmountThreshold : ZERO$1).toString());
...
const PLATFORM_FEE_DENOMINATOR = /*#__PURE__*/JSBI.BigInt(10000);
...
function getQuoteAndSortBasedOnOutAmount({
  ...
  }).filter(Boolean).sort((a, b) => JSBI.greaterThanOrEqual((b === null || b === void 0 ? void 0 : b.quote.outAmount) || ZERO$1, (a === null || a === void 0 ? void 0 : a.quote.outAmount) || ZERO$1) ? 1 : -1);
  return quotes;
...
function processInputRouteSegmentToRoutesInfos({
  ...
  const routesInfo = routes.map(route => {
    ...
    let otherAmountThreshold = ZERO$1;
    ...
    for (const [i, amm] of amms.entries()) {
      try {
        ...
        const platformFee = isLastLeg && platformFeeSupported ? {
          amount: JSBI.divide(JSBI.multiply(quote.outAmount, JSBI.BigInt(platformFeeBps)), PLATFORM_FEE_DENOMINATOR),
        ...
        let amountAfterFees = swapMode === SwapMode.ExactIn ? JSBI.subtract(amountForFees, platformFee.amount) : JSBI.add(amountForFees, platformFee.amount);

        if (JSBI.lessThan(amountAfterFees, ZERO$1)) {
          amountAfterFees = ZERO$1;
        }

        const legOtherAmountThreshold = JSBI.BigInt(swapMode === SwapMode.ExactIn ? new Decimal(amountAfterFees.toString()).mul(1 - slippage / 100).ceil() : new Decimal(amountAfterFees.toString()).mul(1 + slippage / 100).floor());
        const [inAmount, outAmount] = swapMode === SwapMode.ExactIn ? [quote.inAmount, amountAfterFees] : [amountAfterFees, intermediateAmount];
        marketInfos.push({
          amm,
  ...
  }).filter(item => item !== undefined).sort((a, b) => JSBI.greaterThanOrEqual(b.outAmount, a.outAmount) ? 1 : -1); // sort based on which one have better output

  return routesInfo;
}

const codeToErrorRecord = /*#__PURE__*/Object.values(JUPITER_ERRORS).reduce((record, error) => {
...
async function validateTransactionResponse({
  ...
  if (transactionResponse !== null && transactionResponse !== void 0 && (_transactionResponse$ = transactionResponse.meta) !== null && _transactionResponse$ !== void 0 && _transactionResponse$.err) {
    let {
      ...
      programId,
      code
    } = parseErrorForTransaction(transactionResponse);

    if (programId === JUPITER_PROGRAM_ID.toBase58() && code) {
      var _codeToErrorRecord$co;

      message = (_codeToErrorRecord$co = codeToErrorRecord[code]) === null || _codeToErrorRecord$co === void 0 ? void 0 : _codeToErrorRecord$co.msg;
    }
...
function indexedRouteMapToRouteMap(indexedRouteMap) {
  const getMint = index => indexedRouteMap.mintKeys[index]; // generate route map by replacing indexes with mint addresses


  const generatedRouteMap = new Map();
  Object.keys(indexedRouteMap.indexedRouteMap).forEach(key => {
    generatedRouteMap.set(getMint(key), indexedRouteMap['indexedRouteMap'][key].map(index => getMint(index)));
  });
...
class Jupiter {
  ...
  shouldLoadSerumOpenOrders) {
    ...
    this.exchange = async ({
      ...
      blockhashWithExpiryBlockHeight = blockhashWithExpiryBlockHeight || (await this.connection.getLatestBlockhash('confirmed'));
      const {
        transaction: setupTransaction
      } = await setupTransactionBuilder.build(blockhashWithExpiryBlockHeight);
      const {
        transaction
      } = await transactionBuilder.build(blockhashWithExpiryBlockHeight);
      const {
        transaction: cleanupTransaction
      } = await cleanupTransactionBuilder.build(blockhashWithExpiryBlockHeight);
    ...
    try {
      ...
      if (owner.signer) {
        ...
        if (totalTxs > 1) {
          ...
          let i = 0;
          [setupTransaction, swapTransaction, cleanupTransaction] = [setupTransaction ? signedTransactions[i++] : undefined, signedTransactions[i++], cleanupTransaction ? signedTransactions[i++] : undefined];
        } else {
      ...
      if (setupTransaction) {
        ...
        const setupTransactionSender = async () => {
          return await validateTransactionResponse(await transactionSenderAndConfirmationWaiter(this.connection, setupTransaction));
        };

        const setupPromise = setupTransactionSender();
        onTransaction === null || onTransaction === void 0 ? void 0 : onTransaction(setupTxid, totalTxs, 'SETUP', setupPromise);
      ...
      try {
        const swapTransactionSender = async () => {
          return await validateTransactionResponse(await transactionSenderAndConfirmationWaiter(this.connection, swapTransaction));
        };

        const swapPromise = swapTransactionSender();
        onTransaction === null || onTransaction === void 0 ? void 0 : onTransaction(swapTxid, totalTxs, 'SWAP', swapPromise);
        ...
        if (cleanupTransaction) {
          ...
          const cleanupTransactionSender = async () => {
            return validateTransactionResponse(await transactionSenderAndConfirmationWaiter(this.connection, cleanupTransaction));
          };

          const cleanupPromise = cleanupTransactionSender();
          onTransaction === null || onTransaction === void 0 ? void 0 : onTransaction(cleanupTxid, totalTxs, 'CLEANUP', cleanupPromise);
...
export { ALDRIN_SWAP_PROGRAM_ID, ALDRIN_SWAP_V2_PROGRAM_ID, AldrinAmm, CREMA_PROGRAM_ID, CROPPER_PROGRAM_ID, CYKURA_FACTORY_STATE_ADDRESS, CYKURA_PROGRAM_ID, CropperAmm, CykuraAmm, DEVNET_SERUM_DEX_PROGRAM, INDEXED_ROUTE_MAP_URL, IS_DEV, JUPITER_ERRORS, JUPITER_WALLET, Jupiter, LAMPORTS_PER_SIGNATURE, LIFINITY_PROGRAM_ID, MAINNET_SERUM_DEX_PROGRAM, MARINADE_PROGRAM_ID, MARKETS_URL, MERCURIAL_SWAP_PROGRAM_ID, MIN_SEGMENT_SIZE_FOR_INTERMEDIATE_MINTS, MercurialAmm, ORCA_PROGRAM_ID, PENGUIN_PROGRAM_ID, RAYDIUM_AMM_V4_PROGRAM_ID, RaydiumAmm, SABER_ADD_DECIMALS_PROGRAM_ID, SAROS_PROGRAM_ID, SENCHA_PROGRAM_ID, STEPN_PROGRAM_ID, STEP_TOKEN_SWAP_PROGRAM_ID, SWAP_PROTOCOL_TOKENS, SaberAddDecimalsAmm, SaberAmm, SenchaAmm, SerumAmm, SplTokenSwapAmm, SplitTradeAmm, SwapMode, TOKEN_LIST_URL, TransactionBuilder, WHIRLPOOL_PROGRAM_ID, WRAPPED_SOL_MINT, WhirlpoolAmm, getEmptyInstruction, getPlatformFeeAccounts, getRouteInfoUniqueId, getSaberWrappedDecimalsAmms, indexedRouteMapToRouteMap, routeMapToIndexedRouteMap, transactionSenderAndConfirmationWaiter };
