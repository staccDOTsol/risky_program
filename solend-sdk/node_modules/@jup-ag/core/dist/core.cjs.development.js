...
const ORCA_PROGRAM_ID = /*#__PURE__*/new web3_js.PublicKey('9W959DqEETiGZocYWCQPaJ6sBmUzgfxXfqGeTEdp3aQP');
const STEP_TOKEN_SWAP_PROGRAM_ID = /*#__PURE__*/new web3_js.PublicKey('SSwpMgqNDsyV7mAgN9ady4bDVu5ySjmmXejXvy2vLt1');
const PENGUIN_PROGRAM_ID = /*#__PURE__*/new web3_js.PublicKey('PSwapMdSai8tjrEXcxFeQth87xC4rRsa4VA5mhGhXkP');
const SAROS_PROGRAM_ID = /*#__PURE__*/new web3_js.PublicKey('SSwapUtytfBdBn1b9NUGG6foMVPtcWgpRU32HToDUZr');
...
function createProgramAddressSyncUnsafe(seeds, programId) {
  ...
  seeds.forEach(function (seed) {
    if (seed.length > web3_js.MAX_SEED_LENGTH) {
  ...
  let hash = jsSha256.sha256(new Uint8Array(buffer));
  let publicKeyBytes = new BN__default["default"](hash, 16).toArray(undefined, 32); // if (PublicKey.isOnCurve(new Uint8Array(publicKeyBytes))) {
  //   throw new Error(`Invalid seeds, address must fall off the curve`);
  // }

  return new web3_js.PublicKey(publicKeyBytes);
}
...
const [ammAuthority] = /*#__PURE__*/pubkey.findProgramAddressSync([/*#__PURE__*/new Uint8Array( /*#__PURE__*/Buffer.from( /*#__PURE__*/'amm authority'.replace('\u00A0', ' '), 'utf-8'))], RAYDIUM_AMM_V4_PROGRAM_ID);
...
class CropperAmm {
  ...
  constructor(address, accountInfo, params) {
    ...
    this.params = { ...params,
      tokenAFeeAccount: new web3_js.PublicKey(params.tokenAFeeAccount),
      tokenBFeeAccount: new web3_js.PublicKey(params.tokenBFeeAccount)
    };
...
class CykuraAmm {
  ...
  createSwapInstructions(swapParams) {
    ...
    const [,, swapAccountMetas] = this.pool.getOutputAmount(cykuraSdkCore.CurrencyAmount.fromRawAmount(inputToken, swapParams.inAmount));
...
class MarinadeAmm {
  ...
  createSwapInstructions(swapParams) {
    return [swapParams.sourceMint.equals(splToken.NATIVE_MINT) ? createMarinadeFinanceDepositInstruction({ ...swapParams,
      additionalArgs: {
...
function ammFactory(address, accountInfo, params) {
  ...
  if (programId.equals(MAINNET_SERUM_DEX_PROGRAM) || programId.equals(DEVNET_SERUM_DEX_PROGRAM)) {
    ...
    const stableSwap = stableswapSdk.StableSwap.loadWithData(address, accountInfo.data, createProgramAddressSyncUnsafe([address.toBuffer(), nonce], stableswapSdk.SWAP_PROGRAM_ID));
...
async function validateTransactionResponse({
  ...
  if (!transactionResponse) {
    return new optimist.TransactionError('Transaction was not confirmed', txid);
  }
  ...
  if (transactionResponse !== null && transactionResponse !== void 0 && (_transactionResponse$ = transactionResponse.meta) !== null && _transactionResponse$ !== void 0 && _transactionResponse$.err) {
    let {
      ...
      programId,
      code
    } = optimist.parseErrorForTransaction(transactionResponse);
    ...
    return new optimist.TransactionError(message || '', txid, code);
  }
...
class Jupiter {
  ...
  shouldLoadSerumOpenOrders) {
    ...
    this.exchange = async ({
      ...
      const [sourceInstruction, ataInstructions, openOrdersInstructions] = await Promise.all([inputMint.equals(WRAPPED_SOL_MINT) && _wrapUnwrapSOL ? createAndCloseWSOLAccount({
        connection,
        owner,
        amount: routeInfo.swapMode === exports.SwapMode.ExactIn ? routeInfo.amount : routeInfo.otherAmountThreshold
  ...
  static async getRemoteRouteMap({
    ...
    const indexedRouteMap = await (await fetch__default["default"](url)).json();
...
Object.defineProperty(exports, 'TransactionError', {
  enumerable: true,
  get: function () { return optimist.TransactionError; }
});
...
exports.ORCA_PROGRAM_ID = ORCA_PROGRAM_ID;
