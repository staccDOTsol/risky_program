 ...
 * @returns
 */
export async function getAssociatedTokenAddress(tokenMint, owner) {
 ...
 * @returns
 */
export function createAssociatedTokenAccountInstruction(tokenMint, associatedAccount, owner, payer) {
    return Token.createAssociatedTokenAccountInstruction(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, tokenMint, associatedAccount, owner, payer);
}
/**
 ...
 * @returns
 */
export async function getTokenAccount(conn, address) {
    const account = await conn.getAccountInfo(address);
    invariant((account === null || account === void 0 ? void 0 : account.data) !== null, `The token account:${address.toBase58()} data is null`);
 ...
 */
export async function getTokenAccounts(conn, owner) {
    const accounts = await conn.getTokenAccountsByOwner(owner, {
        programId: TOKEN_PROGRAM_ID,
    });
    const accountInfos = [];
    accounts.value.forEach((v) => {
        invariant(v.account.data !== null, `The token account:${v.pubkey.toBase58()} data is null`);
...
export function parseTokenAccount(account) {
    invariant((account === null || account === void 0 ? void 0 : account.data) !== null, `The account data is null`);
    const accountInfo = AccountLayout.decode(account === null || account === void 0 ? void 0 : account.data);
    accountInfo.mint = new PublicKey(accountInfo.mint);
    accountInfo.owner = new PublicKey(accountInfo.owner);
    accountInfo.amount = u64.fromBuffer(accountInfo.amount);
    if (accountInfo.delegateOption === 0) {
