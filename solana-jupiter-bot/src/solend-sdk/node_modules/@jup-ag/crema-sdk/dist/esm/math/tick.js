 ...
 */
export function sqrtPrice2Tick(sqrtPrice) {
    invariant(sqrtPrice.lessThanOrEqualTo(MAX_SQRT_PRICE), `Invalid sqrtPrice: ${sqrtPrice.toString()} Max: ${MAX_SQRT_PRICE.toString()}, too large`);
    invariant(sqrtPrice.greaterThanOrEqualTo(MIN_SQRT_PRICE), `Invalid sqrtPrice: ${sqrtPrice.toString()}, Min: ${MIN_SQRT_PRICE.toString()}, too small`);
 ...
 */
export function tick2SqrtPrice(tick) {
 ...
 * @returns the tick
 */
export function price2Tick(price) {
    invariant(price.lessThan(MAX_PRICE), `Invalid price:${price.toString()} Max: ${MAX_PRICE.toString()},  too large`);
    invariant(price.greaterThan(MIN_PRICE), `Invalid price:${price.toString()} Min: ${MIN_PRICE.toString()}, too small`);
 ...
 * @returns the price
 */
export function tick2Price(tick) {
    invariant(tick >= MIN_TICK && tick <= MAX_TICK, `Invalid tick: ${tick}, must be in range [${MIN_TICK.toString()}, ${MAX_TICK.toString()}]`);
 ...
 */
export function getNearestTick(sqrtPrice, tickSpace) {
 ...
 */
export function getNearestTickBySqrtPrice(sqrtPrice, tickSpace) {
    invariant(tickSpace > 0 && tickSpace % 1 === 0, `Invalid tickSpace:${tickSpace}`);
    invariant(sqrtPrice.lessThan(MAX_SQRT_PRICE), `Invalid sqrtPrice: ${sqrtPrice.toString()} Max: ${MAX_SQRT_PRICE.toString()}, too large`);
    invariant(sqrtPrice.greaterThan(MIN_SQRT_PRICE), `Invalid sqrtPrice: ${sqrtPrice.toString()}, Min: ${MIN_SQRT_PRICE.toString()}, too small`);
 ...
 * @returns the tick or null(if the tick space <= 0)
 */
export function getNearestTickByPrice(price, tickSpace) {
    invariant(tickSpace > 0 && tickSpace % 1 === 0, `Invalid tickSpace:${tickSpace}`);
    invariant(price.lessThan(MAX_PRICE), `Invalid price:${price.toString()} Max: ${MAX_PRICE.toString()},  too large`);
    invariant(price.greaterThan(MIN_PRICE), `Invalid price:${price.toString()} Min: ${MIN_PRICE.toString()}, too small`);
 ...
 */
export function calculateSwapA2B(ticks, currentSqrtPrice, fee, currentLiquity, amountIn) {
 ...
 * @returns amountOut:The amount out of token B, amountUsed:The used of amountIn, afterPrice:The price after calculate, afterLiquity: The liquity after calculate
 */
export function calculateSwapB2A(ticks, currentSqrtPrice, fee, currentLiquity, amountIn) {
    invariant(amountIn.greaterThan(new Decimal(0)), "invalid amount in");
    invariant(currentLiquity.greaterThanOrEqualTo(new Decimal(0)), "invalid liquity");
    invariant(ticks.length > 0, "the ticks is empty");
    //let currentTick = sqrtPrice2Tick(currentSqrtPrice);
    const lastTick = ticks[ticks.length - 1];
    invariant(lastTick !== undefined && currentSqrtPrice.lessThan(lastTick.tickPrice), "out of ticks");
    let liquity = currentLiquity;
    let out = new Decimal(0);
    let remind = amountIn;
    let remindWithFee = new Decimal(0);
...
/** @internal */
export function maxAmountA(lowerSqrtPrice, upperSqrtPrice, liquity) {
    return liquity
        .div(lowerSqrtPrice)
...
/** @internal */
export function maxAmountB(lowerSqrtPrice, upperSqrtPrice, liquity) {
    return liquity
        .mul(upperSqrtPrice.sub(lowerSqrtPrice))
        .toDP(0, Decimal.ROUND_DOWN);
}
/** @internal */
export function swapA2B(upperSqrtPrice, liquity, amountIn) {
    const afterSqrtPrice = liquity.div(amountIn.add(liquity.div(upperSqrtPrice)));
    const delta_increase = amountIn.add(liquity.div(upperSqrtPrice).toDP(0, Decimal.ROUND_DOWN));
    const out = liquity
        .mul(upperSqrtPrice)
        .toDP(0, Decimal.ROUND_DOWN)
...
/** @internal */
export function swapB2A(lowerSqrtPrice, liquity, amountIn) {
    const afterSqrtPrice = amountIn.div(liquity).add(lowerSqrtPrice);
    const delta_increase = amountIn.add(liquity.mul(lowerSqrtPrice).toDP(0, Decimal.ROUND_DOWN));
    const out = liquity
        .div(lowerSqrtPrice)
        .toDP(0, Decimal.ROUND_DOWN)
        .sub(liquity.mul(liquity).div(delta_increase).toDP(0, Decimal.ROUND_DOWN));
 ...
 * @return The ui price
 */
export function tick2UiPrice(tick, baseDecimals, quoteDecimals) {
 ...
 * @return The tick
 */
export function uiPrice2Tick(price, baseDecimals, quoteDecimals) {
 ...
 * @return The ui price
 */
export function lamportPrice2uiPrice(price, baseDecimals, quoteDecimals) {
 ...
 */
export function uiPrice2LamportPrice(price, baseDecimals, quoteDecimals) {
