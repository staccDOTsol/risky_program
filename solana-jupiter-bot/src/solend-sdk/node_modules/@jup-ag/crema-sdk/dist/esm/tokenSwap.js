...
import { createATAInstruction, createMintInstructions, getATAAddress, getOrCreateATA, getTokenAccount, } from "@saberhq/token-utils";
...
import { addUserPositionInstruction, claimInstruction, depositAllTokenTypesInstruction, initializeInstruction, managerClaimInstruction, simulateSwapInstruction, swapInstruction, withdrawAllTokenTypesInstruction, } from "./instructions";
import { depositFixTokenInstruction } from "./instructions/depositFixToken";
import { calculateLiquity, calculateLiquityOnlyA, calculateLiquityOnlyB, calculateSlidTokenAmount, calculateSwapA2B, calculateSwapB2A, calculateTokenAmount, getNearestTickByPrice, lamportPrice2uiPrice, sqrtPrice2Tick, tick2SqrtPrice, tick2UiPrice, uiPrice2LamportPrice, uiPrice2Tick, } from "./math";
import { isPositionsAccount, isTicksAccount, isTokenSwapAccount, MAX_ACCOUNT_POSITION_LENGTH, parsePositionsAccount, parseTicksAccount, parseTokenSwapAccount, POSITIONS_ACCOUNT_SIZE, TOKEN_SWAP_ACCOUNT_SIZE, } from "./state";
...
export class TokenSwap {
    ...
    async load() {
        ...
        accounts.map((item) => {
            if (isTokenSwapAccount(item.account)) {
                ...
                invariant((info === null || info === void 0 ? void 0 : info.data) !== undefined, "The token swap account parse failed");
                this.tokenSwapInfo = {
                    ...info.data,
                ...
                invariant((info === null || info === void 0 ? void 0 : info.data) !== undefined, "The tick account parse failed");
                ...
                invariant((info === null || info === void 0 ? void 0 : info.data) !== undefined, "The position account data parse failed");
        ...
        if (this.authority.toString() === INIT_KEY.toString()) {
            const [authority] = await PublicKey.findProgramAddress([this.tokenSwapKey.toBuffer()], this.programId);
    ...
    static async createTokenSwap({ provider, programId, tokenAMint, tokenBMint, manager, fee, managerFee, tickSpace, tickAccountSize, initializePrice, }) {
        ...
        const [authority, nonce] = await PublicKey.findProgramAddress([swapAccount.publicKey.toBuffer()], programId);
        const ticksAccountLamports = await provider.connection.getMinimumBalanceForRentExemption(tickAccountSize);
        const positionsAccountLarports = await provider.connection.getMinimumBalanceForRentExemption(POSITIONS_ACCOUNT_SIZE);
        const swapAccountLamports = await provider.connection.getMinimumBalanceForRentExemption(TOKEN_SWAP_ACCOUNT_SIZE);
        ...
        const curveType = 0;
        instructions.push(initializeInstruction(programId, swapAccount.publicKey, authority, manager, managerTokenA, managerTokenB, swapTokenA, swapTokenB, ticksAccount.publicKey, positionsAccount.publicKey, nonce, curveType, fee, managerFee, tickSpace, currentSqrtPrice));
        return {
    ...
    async mintPosition(userTokenA, userTokenB, lowerTick, upperTick, liquity, maximumAmountA, maximumAmountB) {
        ...
        invariant(lowerTick < upperTick, "The lowerTick must be less than upperTick");
        const instructions = [];
        ...
        const nftMintInstructions = await createMintInstructions(this.provider, this.authority, positionNftMint.publicKey, 0);
    ...
    async mintPositionFixToken(userTokenA, userTokenB, fixTokenType, lowerTick, upperTick, maximumAmountA, maximumAmountB) {
        ...
        invariant(positionsKey !== null, "The position account space if full");
        // Generate mint positon instruction
        instructions.push(depositFixTokenInstruction(this.programId, this.tokenSwapKey, this.authority, this.provider.wallet.publicKey, userTokenA, userTokenB, this.tokenSwapInfo.swapTokenA, this.tokenSwapInfo.swapTokenB, positionNftMint.publicKey, positionAccount, this.tokenSwapInfo.ticksKey, positionsKey, 0, fixTokenType, lowerTick, upperTick, maximumAmountA, maximumAmountB, new Decimal(0)));
        if (this.tokenSwapInfo.tokenAMint.equals(NATIVE_MINT) &&
            maximumAmountA.greaterThan(0)) {
            const wrapSOLInstructions = await this.wrapSOL(maximumAmountA);
            instructions.unshift(...wrapSOLInstructions.instructions);
    ...
    async decreaseLiquityAtomic(positionId, liquity, minimumAmountA, minimumAmountB, positionAccount = null) {
        const { address: tokenAATA, instruction: tokenAATAInstruction } = await getOrCreateATA({
            provider: this.provider,
            mint: this.tokenSwapInfo.tokenAMint,
            owner: this.provider.wallet.publicKey,
            payer: this.provider.wallet.publicKey,
        });
        const { address: tokenBATA, instruction: tokenBATAInstruction } = await getOrCreateATA({
            provider: this.provider,
    ...
    async swap(userSource, userDestination, direct, amountIn, minimumAmountOut) {
        ...
        const { swapSrc, swapDst } = direct === SWAP_A2B
            ? {
        ...
        const tx = new TransactionEnvelope(this.provider, [
            swapInstruction(this.programId, this.tokenSwapKey, this.authority, this.provider.wallet.publicKey, userSource, userDestination, swapSrc, swapDst, this.tokenSwapInfo.ticksKey, amountIn, minimumAmountOut),
        ]);
        if (this.tokenSwapInfo.tokenAMint.equals(NATIVE_MINT)) {
            if (direct === SWAP_A2B) {
    ...
    async swapAtomic(direct, amountIn, minimumAmountOut) {
        const { srcMint, dstMint } = direct === SWAP_A2B
            ? {
        ...
        const { address: dstATA, instruction: dstATAInstruction } = await getOrCreateATA({
            provider: this.provider,
            mint: dstMint,
            owner: this.provider.wallet.publicKey,
            payer: this.provider.wallet.publicKey,
    ...
    async simulateSwap(amountIn, direction) {
        ...
        const tx = new TransactionEnvelope(this.provider, [
            simulateSwapInstruction(this.programId, this.tokenSwapKey, this.tokenSwapInfo.ticksKey, amountIn, direction),
        ]);
    ...
    async claim(positionId, userTokenA, userTokenB, positionAccount = null) {
        ...
        const position = await this._checkUserPositionAccount(positionId, positionAccount);
        const tx = new TransactionEnvelope(this.provider, [
            claimInstruction(this.programId, this.tokenSwapKey, this.authority, this.provider.wallet.publicKey, this.tokenSwapInfo.swapTokenA, this.tokenSwapInfo.swapTokenB, userTokenA, userTokenB, positionId, position.positionAccount, this.tokenSwapInfo.ticksKey, position.positionInfo.positionsKey, position.positionInfo.index),
        ]);
        if (this.tokenSwapInfo.tokenAMint.equals(NATIVE_MINT)) {
            const unwrapSOLTx = await this.unwrapSOL(userTokenA);
            tx.instructions.push(...unwrapSOLTx.instructions);
    ...
    async managerClaim() {
        ...
        const tx = new TransactionEnvelope(this.provider, [
            managerClaimInstruction(this.programId, this.tokenSwapKey, this.authority, this.provider.wallet.publicKey, this.tokenSwapInfo.swapTokenA, this.tokenSwapInfo.swapTokenB, this.tokenSwapInfo.managerTokenA, this.tokenSwapInfo.managerTokenB),
        ]);
        if (this.tokenSwapInfo.tokenAMint.equals(NATIVE_MINT)) {
    ...
    async addPositionsAccount() {
        ...
        const positionsAccount = Keypair.generate();
        const lamports = await this.provider.connection.getMinimumBalanceForRentExemption(POSITIONS_ACCOUNT_SIZE);
    ...
    async getUserPositions(owner = this.provider.wallet.publicKey) {
        ...
        const tokenAccounts = await getTokenAccounts(this.provider.connection, owner);
        const positions = [];
    ...
    calculateLiquityByTokenA(tickLower, tickUpper, desiredAmountA, currentSqrtPrice = this.tokenSwapInfo.currentSqrtPrice) {
        ...
        invariant(this.isLoaded, "The token swap not load");
        invariant(currentTick <= tickUpper, "when current price greater than upper price, can only add token b");
        if (currentTick < tickLower) {
            ...
            const res = calculateLiquity(tickLower, tickUpper, desiredAmountA, currentSqrtPrice, 0);
            return {
    ...
    calculateLiquityByTokenB(tickLower, tickUpper, desiredAmountB, currentSqrtPrice = this.tokenSwapInfo.currentSqrtPrice) {
        const currentTick = sqrtPrice2Tick(currentSqrtPrice);
        invariant(this.isLoaded, "The token swap not load");
        invariant(currentTick >= tickLower, "when current price less than lower price, can only add token a");
        if (currentTick > tickUpper) {
            ...
            const res = calculateLiquity(tickLower, tickUpper, desiredAmountB, currentSqrtPrice, 1);
            return {
    ...
    calculateLiquityWithSlid({ lowerTick, upperTick, amountA = null, amountB = null, slid = new Decimal(0.01), }) {
        ...
        const slidRes = calculateSlidTokenAmount(lowerTick, upperTick, liquity, this.tokenSwapInfo.currentSqrtPrice, slid);
        return {
    ...
    calculatePositionValue(positionId) {
        ...
        invariant(positionInfo !== undefined, `The position:${positionId.toBase58()} not found`);
        const { amountA, amountB } = calculateTokenAmount(positionInfo.lowerTick, positionInfo.upperTick, positionInfo.liquity, this.tokenSwapInfo.currentSqrtPrice);
        return {
    ...
    calculatePositionValueWithSlid(positionId, percentage = new Decimal(1), slid = new Decimal(0.01)) {
        invariant(this.isLoaded, "The token swap not load");
        invariant(percentage.greaterThan(0) && percentage.lessThanOrEqualTo(1), `Invalid pencentage:${percentage.toString()}`);
        const positionInfo = this.getPositionInfo(positionId);
        invariant(positionInfo !== undefined, `The position:${positionId.toBase58()} not found`);
        const liquity = positionInfo.liquity.mul(percentage).toDecimalPlaces(0);
        const res = calculateSlidTokenAmount(positionInfo.lowerTick, positionInfo.upperTick, liquity, this.tokenSwapInfo.currentSqrtPrice, slid);
        return {
            liquity,
            ...res,
        };
    ...
    preClaim(positionId) {
        ...
        invariant(lowerTick !== null, `The position lower tick:${positionInfo.lowerTick} not found`);
        invariant(upperTick !== null, `The position upper tick:${positionInfo.upperTick} not found`);
    ...
    preSwapA(amountIn) {
        invariant(this.isLoaded, "The token swap not load");
        const res = calculateSwapA2B(this.ticks, this.tokenSwapInfo.currentSqrtPrice, this.tokenSwapInfo.fee, this.tokenSwapInfo.currentLiquity, amountIn);
    ...
    preSwapB(amountIn) {
        invariant(this.isLoaded, "The token swap not load");
        const res = calculateSwapB2A(this.ticks, this.tokenSwapInfo.currentSqrtPrice, this.tokenSwapInfo.fee, this.tokenSwapInfo.currentLiquity, amountIn);
    ...
    uiPrice() {
        invariant(this.isLoaded, "The token swap not load");
        return lamportPrice2uiPrice(this.tokenSwapInfo.currentSqrtPrice.pow(2), this.tokenSwapInfo.tokenADecimals, this.tokenSwapInfo.tokenBDecimals);
    }
    uiReversePrice() {
        invariant(this.isLoaded, "The token swap not load");
    ...
    uiPrice2SwapPrice(price) {
        invariant(this.isLoaded, "The token swap not load");
        return uiPrice2LamportPrice(price, this.tokenSwapInfo.tokenADecimals, this.tokenSwapInfo.tokenBDecimals);
    }
    uiPrice2SwapSqrtPrice(price) {
        invariant(this.isLoaded, "The token swap not load");
        return uiPrice2LamportPrice(price, this.tokenSwapInfo.tokenADecimals, this.tokenSwapInfo.tokenBDecimals).sqrt();
    }
    uiReversePrice2SwapPrice(price) {
        invariant(this.isLoaded, "The token swap not load");
        return uiPrice2LamportPrice(new Decimal(1).div(price), this.tokenSwapInfo.tokenADecimals, this.tokenSwapInfo.tokenBDecimals);
    }
    uiReversePrice2SwapSqrtPrice(price) {
        invariant(this.isLoaded, "The token swap not load");
        return uiPrice2LamportPrice(new Decimal(1).div(price), this.tokenSwapInfo.tokenADecimals, this.tokenSwapInfo.tokenBDecimals).sqrt();
    }
    uiPrice2Tick(price) {
        invariant(this.isLoaded, "The token swap not load");
        return uiPrice2Tick(price, this.tokenSwapInfo.tokenADecimals, this.tokenSwapInfo.tokenBDecimals);
    }
    uiReversePrice2Tick(price) {
        invariant(this.isLoaded, "The token swap not load");
        return uiPrice2Tick(new Decimal(1).div(price), this.tokenSwapInfo.tokenADecimals, this.tokenSwapInfo.tokenBDecimals);
    }
    uiPrice2NearestTick(price) {
        invariant(this.isLoaded, "The token swap not load");
        const swapPrice = uiPrice2LamportPrice(price, this.tokenSwapInfo.tokenADecimals, this.tokenSwapInfo.tokenBDecimals);
        return this.getNearestTickByPrice(swapPrice);
    }
    uiReversePrice2NearestTick(price) {
        invariant(this.isLoaded, "The token swap not load");
        const swapPrice = uiPrice2LamportPrice(new Decimal(1).div(price), this.tokenSwapInfo.tokenADecimals, this.tokenSwapInfo.tokenBDecimals);
        return this.getNearestTickByPrice(swapPrice);
    }
    tick2UiPrice(tick) {
        invariant(this.isLoaded, "The token swap not load");
        return tick2UiPrice(tick, this.tokenSwapInfo.tokenADecimals, this.tokenSwapInfo.tokenBDecimals);
    }
    tick2UiReversePrice(tick) {
        invariant(this.isLoaded, "The token swap not load");
        return new Decimal(1).div(tick2UiPrice(tick, this.tokenSwapInfo.tokenADecimals, this.tokenSwapInfo.tokenBDecimals));
    }
    ...
    calculateEffectivTick(lowerPrice, upperPrice) {
        invariant(upperPrice.greaterThan(lowerPrice), "The upper price must greater than lower price");
    ...
    calculateFixSideTokenAmount(lowerTick, upperTick, amountA, amountB, balanceA, balanceB, slid = new Decimal(0.01)) {
        ...
        if (amountA !== null) {
            ...
            invariant(desiredAmountB.lessThanOrEqualTo(balanceB), "The fixed amount of token A is too large");
            slidSqrtPrice = this.tokenSwapInfo.currentSqrtPrice.mul(new Decimal(1).add(slid).sqrt());
            ...
            invariant(amountB !== null, "You must specified the amount of token A or token B");
            ...
            desiredDeltaLiquity = res.liquity;
            invariant(desiredAmountA.lessThanOrEqualTo(balanceA), "The fixed amount of token B is too large");
            slidSqrtPrice = this.tokenSwapInfo.currentSqrtPrice.mul(new Decimal(1).sub(slid).sqrt());
    ...
    async wrapSOL(amount) {
        ...
        const tx = new TransactionEnvelope(this.provider, []);
        const { address: ataAddress, instruction: ataInstruction } = await getOrCreateATA({
            provider: this.provider,
            mint: NATIVE_MINT,
            owner: this.provider.wallet.publicKey,
            payer: this.provider.wallet.publicKey,
        ...
        tx.instructions.push(SystemProgram.transfer({
            fromPubkey: this.provider.wallet.publicKey,
            toPubkey: ataAddress,
    ...
    async unwrapSOL(ataAddress, dest = this.provider.wallet.publicKey) {
        ...
        const tx = createCloseAccountInstruction(ataAddress, dest, this.provider.wallet.publicKey);
    ...
    async _checkUserPositionAccount(positionId, positionAccount) {
        ...
        const positionInfo = this.getPositionInfo(positionId);
        invariant(positionInfo !== undefined, `Position:${positionId.toString()} not found`);
        ...
        const positionAccountInfo = await getTokenAccount(this.provider, positionAccount);
        invariant(positionAccountInfo.mint.toString() === positionId.toString(), `Invalid position account:${positionAccount.toBase58()}`);
        invariant(positionAccountInfo.amount.toNumber() === 1, `You not hold this position:${positionId.toBase58()}`);
        return {
            positionInfo,
            positionAccount,
        };
