...
__export(src_exports, {
  ...
  estimateAprsForPriceRange: () => estimateAprsForPriceRange,
...
function getDefaultConnection(network) {
  switch (network) {
    case "mainnet" /* MAINNET */:
      return new import_web34.Connection("https://ssc-dao.genesysgo.net", "processed");
    case "devnet" /* DEVNET */:
...
function getWhirlpoolsConfig(network) {
  switch (network) {
    case "mainnet" /* MAINNET */:
      return new import_web35.PublicKey("2LecshUwdy9xi7meFgHtFJQNSKk4KdTrcpvaB56dP2NQ");
    case "devnet" /* DEVNET */:
      return new import_web35.PublicKey("847gd7SckNwJetbjf22ktip9vAtKWPMY4ntdr6CdCsJj");
    default:
      throw new Error(`type ${network} is an Unknown network`);
  }
}
...
var OrcaPool = class {
  ...
  derivePDA(tokenMintA, tokenMintB, tickSpacing) {
    ...
    return (0, import_whirlpool_client_sdk9.getWhirlpoolPda)(this.dal.programId, this.dal.whirlpoolsConfig, toPubKey(_tokenMintA), toPubKey(_tokenMintB), tickSpacing);
  }
...
function estimateAprsForPriceRange(pool, tokenPrices, fees24h, tickLowerIndex, tickUpperIndex) {
  ...
  const { minTokenA, minTokenB } = getRemoveLiquidityQuote({
    positionAddress: import_web39.PublicKey.default,
    tickCurrentIndex,
    sqrtPrice,
    tickLowerIndex,
    tickUpperIndex,
  ...
  const feesPerYear = new import_decimal5.default(fees24h).mul(365);
...
function getTokenValue(tokenAmount, tokenDecimals, tokenPrice) {
  return DecimalUtil.adjustDecimals(new import_decimal5.default(tokenAmount.toString()), tokenDecimals).mul(tokenPrice);
}
...
function sqrtPriceX64ToPrice(sqrtPriceX64, decimalsA, decimalsB) {
  return (0, import_whirlpool_client_sdk11.fromX64)(sqrtPriceX64).pow(2).mul(import_decimal6.default.pow(10, decimalsA - decimalsB));
}
function priceToSqrtX64(price, decimalsA, decimalsB) {
  return (0, import_whirlpool_client_sdk11.toX64)(price.mul(import_decimal6.default.pow(10, decimalsB - decimalsA)).sqrt());
}
...
function getStartTickIndex(tickIndex, tickSpacing, offset = 0) {
  const realIndex = Math.floor(tickIndex / tickSpacing / import_whirlpool_client_sdk11.TICK_ARRAY_SIZE);
  const startTickIndex = (realIndex + offset) * tickSpacing * import_whirlpool_client_sdk11.TICK_ARRAY_SIZE;
  return startTickIndex;
}
...
async function convertPositionDataToUserPositionData(dal, walletAddress, refresh) {
  ...
  for (const address of positionAddresses) {
    ...
    for (const [index, { mint, vault }] of whirlpool.rewardInfos.entries()) {
      ...
      const decimals = !mint.equals(import_web311.PublicKey.default) && !vault.equals(import_web311.PublicKey.default) ? (_c = await dal.getMintInfo(mint, false)) == null ? void 0 : _c.decimals : void 0;
    ...
    pools.forEach((pool) => {
      if (pool) {
        ...
        pool.rewardInfos.forEach(({ mint, vault }) => {
          if (!mint.equals(import_web311.PublicKey.default) && !vault.equals(import_web311.PublicKey.default)) {
            allMintInfos.add(mint.toBase58());
          }
        });
      }
...
// src/client.ts
import_decimal7.default.set({ precision: 40, toExpPos: 40, toExpNeg: -20, rounding: 1 });
var OrcaWhirlpoolClient = class {
  ...
  async getTokenPrices(poolAddresses, baseTokenMint, baseTokenUSDPrice = new import_decimal7.default(1), otherBaseTokenMints = [import_spl_token11.NATIVE_MINT], refresh = true) {
...
var PROGRAM_ID = new import_web312.PublicKey("whirLbMiicVdio4qvUfM5KAg6Ct8VwpYzGff3uctyCc");
...
function getTickArrayPublicKeysForSwap(tickCurrentIndex, targetSqrtPriceX64, tickSpacing, poolAddress, tickArrays, programId, aToB) {
  ...
  const tickArrayAddresses = [
    ...
    import_web312.PublicKey.default,
    import_web312.PublicKey.default
  ];
  while (currentStartTickIndex != targetStartTickIndex && count < 3) {
    const nextStartTickIndex = TickUtil.getStartTickIndex(nextInitializableTickIndex, tickSpacing, offset * count);
