...
const useMangoStats = () => {
    ...
    const fetchHistoricalStats = async () => {
      ...
      try {
        const response = await fetch(
          `https://mango-transaction-log.herokuapp.ca/v3/stats/spot_stats_hourly?mango-group=${mangoGroupName}`
        )
  ...
  useEffect(() => {
    const fetchHistoricalPerpStats = async () => {
      ...
      try {
        const response = await fetch(
          `https://mango-transaction-log.herokuapp.ca/v3/stats/perp_stats_hourly?mango-group=${mangoGroupName}`
        )
        const stats = await response.json()
        setPerpStats(stats)
