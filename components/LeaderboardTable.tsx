...
const LeaderboardTable = ({ range = '29' }) => {
  ...
  const fetchPnlLeaderboard = async () => {
    ...
    try {
      const response = await fetch(
        `https://mango-transaction-log.herokuapp.ca/v3/stats/pnl-leaderboard?start-date=${dayjs()
          .utc()
          .hour(0)
          .minute(0)
  ...
  const fetchPerpPnlLeaderboard = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `https://mango-transaction-log.herokuapp.ca/v3/stats/perp-pnl-leaderboard?start-date=${dayjs()
          .utc()
          .hour(0)
          .minute(0)
          .subtract(parseInt(range), 'day')
  ...
  const fetchSpotPnlLeaderboard = useCallback(async () => {
    setLoading(true)
    const response = await fetch(
      `https://mango-transaction-log.herokuapp.ca/v3/stats/spot-pnl-leaderboard?start-date=${dayjs()
        .hour(0)
        .minute(0)
        .utc()
        .subtract(parseInt(range), 'day')
