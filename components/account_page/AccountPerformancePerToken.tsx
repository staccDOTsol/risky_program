...
const AccountPerformance = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      const promises = [
        fetch(
          `https://mango-transaction-log.herokuapp.ca/v3/stats/account-performance-per-token?mango-account=${mangoAccountPk}&start-date=${dayjs()
            .subtract(parseInt(performanceRange), 'day')
            .format('YYYY-MM-DD')}`
        ),
        fetch(
          `https://mango-transaction-log.herokuapp.ca/v3/stats/volumes-by-mango-account?mango-account=${mangoAccountPk}&start-date=${dayjs()
            .subtract(parseInt(performanceRange), 'day')
            .format('YYYY-MM-DD')}`
        ),
      ]
